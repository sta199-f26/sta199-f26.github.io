...
3.  Quarto: The tool you'll use to create reproducible computational documents that contain both your narrative (i.e., words in English) and your code (i.e., code in R). Every assignment you hand in will be created using Quarto.
    ...
    Instead, the words you would write in a word processor, as well as the code, will go into a Quarto document. When you *render* the document (more on what this means later) you will get a document out that has your words, your code, and the output of that code.
...
-   Click `STA199` to log into the Docker container. You should now see the Positron environment.
...
mtk: Not sure if this happens on the first go but i think it does. you just have to wait until everything stops loading (you can see the text at the very bottom of the screen)

Select the **Accounts** icon in the lower left of your screen. It should have a red 1 in the lower right of the icon. It may take a minute for everything in your container to finish loading before the red 1 appears. Check the status text at the very bottom of the screen if you don't see it right away.

Select **Sign in with GitHub to use GitHub Pull Requests (1)**. A pop-up with a code will appear. 

Select **Copy & Continue to Browser**. Your browser may block pop-ups, in which case you may get a message to allow pop-ups for this website. If so, select **Retry** to allow pop-ups for your container website.

You should now be on a GitHub webpage. Use your credentials to sign into your GitHub account. 

Use control-v on PC (or command-v on Mac) to paste the code into the GitHub screen.

[mtk: shoot i didn't get a screenshot of this step. also the paste didn't work the first time -- so idk if they should just highlight the text first and copy it. it did work the third time. i'm not sure if the allow pop-up step interrupted the copy somehow.]

Now authorize access between Positron and GitHub by selecting the green **Authorize Visual-Studio-Code** button. You may need to sign in to GitHub again to confirm access. [mtk: should i check this?]

You should now get a screen stating "Congratulations, you're all set!"
...
mtk: i think all of this stuff doesn't happen when using https but should confirm first (check the next paragraph too). A pop-up may appear stating: "github.com" has fingerprint "lotsoflettersandsymbols." Are you sure you want to continue connecting? If so, type "yes" and hit return.

A Git: Warning pop-up may appear stating that a known host was permanently added. If so, select `Open Git Log` to continue. 

    ![](images/lab-0/lab-0-clone.png){fig-alt="Cloning a GitHub repository in RStudio." width="600" fig-align="center"}

-   Since this is the first time you are connecting to GitHub from this Positron instance, you will be asked to confirm the authenticity of the host.
    This will happen in a pop-up window like the one below.
    Type **yes** in the box and hit *Enter*.
    
    [mtk: replace this image. actually i don't think this happens when you use the https link -- so this bit can go.]
