...
5.  GitHub: A web hosting service for the Git version control system that also allows for transparent collaboration between team members.
    ...
    ::: callout-note
    Git is a version control system (like "Track Changes" features from Microsoft Word but more powerful) and GitHub is the home for your Git-based work on the internet (like DropBox but much better).
    :::
...
Select the `Accounts` icon in the lower left of your screen. It should have a red 1 in the lower right of the icon. Note: you may need to wait a minute for everything to load before the red 1 appears.

Select `Sign in with GitHub to use GitHub Pull Requests (1)`. A pop-up with a code will appear. 

Select `Copy & Continue to Browser`. Your browser may block pop-ups, in which case you may get a mesage to allow pop-ups for this website. If so, select `Retry` to allow pop-up for your container website.

You should now be on a GitHub webpage. Use your credentials to sign into your GitHub account. 

Use control-v on PC (or command-v on Mac) to paste the code into the GitHub screen [mtk: shoot i didn't get a screenshot of this step. also the paste didn't work the first time -- so idk if they should just highlight the text first and copy it. it did work the third time. i'm not sure if the allow pop-up step interrupted the copy somehow.]
...
Navigate back to your container tab. If you click on "Accounts" you should now see that you are signed into your GitHub accont within Positron, as your Github username appears followed by "(GitHub)".

mtk: not sure if you'll need to introduce yourself to github yet.skipping to cloning a repo
...
-   Paste the URL of the lab-0 repo into the dialog box *Git repository URL*. Again, please make sure to have *HTTPS* highlighted under *Clone* when you copy the address.

-   Click *OK*. 

mtk: i think all of this stuff doesn't happen when using https. A pop-up may appear stating: "github.com" has fingerprint "lotsoflettersandsymbols." Are you sure you want to continue connecting? If so, type "yes" and hit return.

A Git: Warning pop-up may appear stating that a known host was permanently added. If so, select `Open Git Log` to continue. 

MTK: hmm: that didn't work. see screenshot.and they may need to do those steps again?
