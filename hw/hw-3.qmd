...
c\.
Create a new dataset by joining the `us_inflation` dataset with the `cpi_expenditures` dataset.
...
In class, we worked with the `sales_taxes` dataset and joined it to another dataset called `us_regions` so that we ended up with a data frame containing information on each state (plus DC), sales tax rate in that state, and the region the state is in. While DC was in our first dataset, it was not in the second, resulting in an `NA` value for the `region` variable in the joined dataset. We discussed that there are three approaches we could take to address this issue before calculating the average sales tax of states in each region.
...
b\. 
Apply the three approaches we discussed in class to address the issue that DC has an `NA` value for `region` in the joined dataset, and then calculate the average sales tax of states in each region.
