---
title: HW 1
subtitle: Exploring Spotify charts and ...
...
Create an overlaid density plot that compares the distribution of weekly streams (`streams`) by `genre`. Include only genres with at least 10 songs on the chart. Use filled density curves with transparency so that overlapping distributions remain visible, and use a colorblind-friendly color palette. Then, highlight one feature of weekly streams that this plot reveals in 1-2 sentences.

::: callout-tip
#### Hints

- x-axis: You can modify the x-axis labels to use commas in a `scale_x_continuous()` layer. The **scales** package offers a handy function for formatting numbers, `label_comma()`. You can use this in the `labels` argument of `scale_x_continuous()`. In, `label_comma()`, set a `scale` and `suffix`.
...
Did you select your pages on Gradescope? [^2] You don't need to write an answer for this question.

[^2]: Selecting your pages for each question makes the grading process much smoother.
