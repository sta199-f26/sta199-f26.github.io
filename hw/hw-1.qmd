...
- Go to the course organization at [github.com/sta199-f26](https://github.com/sta199-f26) organization on GitHub. Click on the repo with the prefix **hw-1**. It contains the starter documents you need to complete the homework.
- Click on the green **CODE** button, select **Use SSH**. Click on the clipboard icon to copy the repo URL.
- In Positron, go to *File* ➛ *New Folder from Git...*
- Copy and paste the URL of your assignment repo into the dialog box *Git Repository URL*. Again, please make sure to have *SSH* highlighted under *Clone* when you copy the address.
- Click *OK*, and the files from your GitHub repo will be displayed in the *Explorer* view in Positron

Open the `hw-1.qmd` template Quarto file and update the author field to add your name (first and last). Render the document. Examine the rendered document and make sure your name is updated in the document. Commit and push your changes with a meaningful commit message and push to GitHub.
...
We will use the `midwest` data frame for this part of the homework. It is part of the **ggplot2** R package, so the `midwest` data set is automatically loaded when you load the tidyverse package.
...
For each statement below, replace `Yes/No` with either **Yes** or **No**.

| Response | Statement |
|:--------------------------------:|--------------------------------------|
| Yes/No | I used an LLM / generative AI tool for this assignment. |
| Yes/No | I used an LLM / generative AI tool to clarify the question(s). |
| Yes/No | I used an LLM / generative AI tool clarifying questions to better understand a concept. |
| Yes/No | I asked an LLM / generative AI tool to help write code. |
| Yes/No | I gave an LLM / generative AI tool my code and asked it to help me fix it. |
| Yes/No | I asked an LLM / generative AI tool about an error or why my code behaved in an unexpected way. |
| Yes/No | I pasted the question(s) prompt into an LLM / generative AI tool and asked for help, but I wrote my answer myself. |
| Yes/No | I pasted the question(s) prompt into an LLM / generative AI tool and copied and pasted at least part of its response into my Quarto document. |
| Yes/No | Other (describe below). |

If you answered **Yes** to any of the above statements list:

- The name of the model (e.g., ChatGPT GPT-5.5, Claude Sonnet 4, Gemini 2.5 Pro).
- A link to the chat thread (if available).
- If the link is not available, list the prompt(s) you used.
- Specify the question(s) you used an LLM or generative AI tool.

Additionally, make sure to cite any other non-AI sources you used to help you complete the assignement.

Two sample citations are:

All questions: Claude Sonnet 4, (give url)

Alternatively,

- Question 5: ChatGPT GPT-5.5. Prompt: Write R code to find the rows for which the column country is USA and only keep the columns country and continent.

- Question 7: ChatGPT GPT-5.5. Prompt: Why did my code have an error. (and paste some code)
