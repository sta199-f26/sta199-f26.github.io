...
The [NBA Draft Combine](https://en.wikipedia.org/wiki/NBA_draft_combine) is a multi-day event held every May before the NBA draft in June.
...
The data are stored in two sheets in a single Excel file called `nba-draft-combine-2627.xlsx`, located in your `data` folder.
...
a. Determine which, if any, players are in the stats data frame but not in the picks data frame. These are players present in the combine data but absent from the supplied draft-pick data. Their absence does not necessarily mean they went undrafted. Display your output.
...
c. Briefly, in at most 3 sentences, describe your findings about the relationship between weights and positions of players in this combine dataset based on your results from parts (a) and (b).
...
![](images/hw-4/draft-weight-sprint-1.png){alt="Draft weight vs. three-quarter sprint time, with Duke players highlighted in blue and labeled with their names."}
...
::: openai
This would be a good time to get feedback from AI if you haven't already!
Just launch Codex in Positron and ask it to review your answer for any question(s) you haven't yet reviewed.
:::
...
::: callout-note
The Pan American Games feature athletes from both North and South American countries.
...
b. Join the two data frames based on `country` and `entity` variables in their respective datasets, and save the resulting data frame as a new data frame, giving it a short but evocative name. Display the first ten rows of the new data frame. Make sure that your joined data frame contains all countries in the `gymnastics` data frame.
...
We will be checking these to make sure you have been practicing committing and pushing changes.
:::
...
Submit your final version of `hw-4.pdf` to Gradescope.
