...
> “Look at that, there is a county type containing at least six counties where less than half of adults earn a family-sustaining wage!”

In a single pipeline, discover all county types that could fill in the blank.
Your response should be a data frame with only the county type and the number of counties where `p_family_sustaining_wage` is less than 0.50, followed by a sentence identifying the county types that satisfy the statement.
...
The dataset records several education categories separately, so combine them into one measure of the percentage of 25–44-year-olds with an associate degree or higher.

Create a new variable called `p_edu_he` (short for "percentage with higher education") that is the sum of the percentages of
...
In a single pipeline, calculate the interquartile range (IQR) of `p_edu_he` for each `county_type`.
Then, use the IQR and the rule "points that are 1.5 times IQR away from the 25th and 75th percentiles are considered outliers" rule to identify counties that are outliers in the distribution of `p_edu_he` for their county type.
The output should be a data frame with the following three columns, in this order: `county`, `county_type`, and `p_edu_he`, and arranged in ascending order of county type and descending order of percentage with higher education.
...
Finally, examine how education and economic opportunity vary together.
This comparison can help us describe whether counties with higher levels of higher education also tend to have a higher percentage of adults earning a family-sustaining wage, while remembering that a county-level association is not evidence that one variable causes the other.

Create a scatter plot with percentage of 25-44-year-olds with higher education (`p_edu_he`) on the x-axis and percentage of adults earning a family-sustaining wage (`p_family_sustaining_wage`) on the y-axis. Describe the relationship between these two variables. In your description, make sure to comment on the direction, form, strength, and any unusual observations. For unusual observations, identify the counties that correspond to these observations, and make sure to include the code you used to identify them as part of your answer. Your code should output the county name, percentage with higher education, and percentage earning a family-sustaining wage for each unusual observation.

Requirements for the plot, in addition to the requirements in the overall assignment guidelines, are as follows:

- The x-axis should be formatted as percentages, e.g., 30%, not 0.3.
- The y-axis should be formatted as percentages, e.g., 50%, not 0.5.
...
The previous questions treated higher educational attainment as a numerical variable.
Now use it to identify the counties at the upper end of the distribution and explore whether those counties are concentrated in particular county types.

a. In a single pipeline, calculate the cutoff for the top 20% of counties based on `p_edu_he` and save it as `top_20_edu_cutoff` as a single numeric value.
This cutoff is the 80th percentile of `p_edu_he`.

b. In a single pipeline, create a new variable called `edu_percentile` in `nc_county` that indicates whether a county is in the `"Top 20%"` or `"Bottom 80%"` of counties based on `p_edu_he`.
Use `top_20_edu_cutoff` to make this classification, and ensure that the levels of `edu_percentile` are ordered so that `"Top 20%"` comes before `"Bottom 80%"`.

c. In a single pipeline, calculate the proportion of counties in each `edu_percentile` group for each `county_type`.
Then, create a segmented bar plot with one bar for each `county_type`, filled according to `edu_percentile`.
The y-axis should range from 0 to 1 and represent the proportion of counties in each county type.

Based on your table and plot, describe which county types have the largest proportions of counties in the top 20% of higher educational attainment.
Remember that these are descriptive county-level comparisons and do not establish that county type causes educational attainment.
