...
::: wooclap
What does each point represent in this visualization?
:::
...
::: wooclap
The plot was created, but R displayed the warning below. Is this an error? What happened to the 94 observations?
:::
...
`age` is missing for 94 respondents, so ggplot2 could not place them on the y-axis. It omitted those rows and warned us rather than silently dropping them.
:::
...
gss |>
  ...
  ggplot(aes(x = marital_status, fill = happiness)) +
  geom_bar() +
  labs(x = "Marital status", y = "Count", fill = "Happiness") +
